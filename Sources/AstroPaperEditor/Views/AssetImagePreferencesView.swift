    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                .disabled(!store.hasProject || store.isAssetScanRunning || store.isAssetOptimizeRunning)
                ...
                .disabled(!store.hasProject || store.isAssetScanRunning || store.isAssetOptimizeRunning || preview?.unusedImages.isEmpty != false)
            }
        ...
        .onAppear {
            if store.hasProject, preview == nil, !store.assetCleanupMessage.isEmpty {
