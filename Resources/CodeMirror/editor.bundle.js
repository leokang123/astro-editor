(() => {
      ...
      return new _ViewPlugin(nextPluginID++, create, eventHandlers, eventObservers, (plugin2) => {
        ...
        if (deco)
          ext.push(decorations.of((view2) => {
            let pluginInst = view2.plugin(plugin2);
  ...
  function bindHandler(plugin2, handler) {
    return (view2, event) => {
      try {
        return handler.call(plugin2, event, view2);
      } catch (e) {
  ...
  var plugin = /* @__PURE__ */ ViewPlugin.fromClass(class {
    ...
    update(update) {
      let { view: view2 } = update;
      let height = view2.viewState.editorHeight - view2.defaultLineHeight - view2.documentPadding.top - 0.5;
  ...
  function scrollPastEnd() {
    return [plugin, contentAttributes.of((view2) => {
      var _a2;
      return ((_a2 = view2.plugin(plugin)) === null || _a2 === void 0 ? void 0 : _a2.attrs) || null;
    })];
  }
  ...
  // Resources/CodeMirror/editor.js
  var currentDocumentID = "";
  var suppressChange = false;
  var pendingTextChange = 0;
  var currentFindQuery = "";
  var post = (name2, body) => {
    window.webkit?.messageHandlers?.[name2]?.postMessage(body);
  };
  var sourcePositionForScrollTop = () => {
    if (!view) return 1;
  ...
  var searchHighlightEffect = StateEffect.define();
  ...
  var reportFindStatus = (query, replacementCount = null) => {
  ...
  var updateSearchHighlights = (query, replacementCount = null) => {
    const ranges = findRanges(query);
  ...
  var findRange = (query, direction, moveFromSelection) => {
    const ranges = findRanges(query);
    if (!ranges.length) return null;
    const selection = view.state.selection.main;
    if (direction < 0) {
      const start2 = Math.max((moveFromSelection ? selection.from : selection.to) - 1, 0);
      for (let index = ranges.length - 1; index >= 0; index -= 1) {
        if (ranges[index].from <= start2) return ranges[index];
      }
  ...
  var selectFindRange = (query, direction, moveFromSelection) => {
  ...
  var replaceCurrent = (query, replacement) => {
  ...
  var replaceAll = (query, replacement) => {
  ...
  var createState = (text) => EditorState.create({
    doc: text,
